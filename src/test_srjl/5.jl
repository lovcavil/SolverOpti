...
# —— 2. 在 Main 里定义自定义 loss —— 
"""
my_curve_loss(ex, dataset, options)：
  - dataset.X: 2×n
  - dataset.y: Vector{Vector{Float64}} of length n
  每条样本 i：
    1) 构造 3×m 的 feat 矩阵 = [c1_i; c2_i; x_vals]
    2) eval_tree_array 得到预测曲线 pred (长度 m)
    3) 用 MAE(pred, Y_true_i) 累加
  最后平均得到标量 loss
"""
